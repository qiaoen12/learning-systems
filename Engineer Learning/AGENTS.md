...
本目录保存 Corpus-First 3.3 学习路线的执行版、必要个人上下文、手写记录和历史快照。当前路线以「先读后写、判据先行、生产只读」为核心；代码实现只落在 `corpus-guard`，不落在本研究目录。
...
- 不修改或删除 `../AI student/` 的历史材料，它是冻结的史前史。唯一例外是归档标注：当旧目录里的文件会对代理下达过期指令时，允许加墓碑或把断言改成过去式，不得改动其余内容。`../AI student/AGENTS.md` 已按此处理。
- 不代替用户填写 `records/`。日志、根因判断、规则取舍必须由学习者本人写。

## 当前路线硬约束

- 读能力优先于写能力。
- L0.5 的语法输入累计满 6 小时即硬停并转读代码。它约束的是**输入这个动作**，**不是 L0.5 的通过条件**。这是待记录的假设，不是对学习者能力的评价。
- `questions.md` 的条数和语法输入小时数都是诊断指标，不用于判定层级通过。
...
- **入口阀门冷启动锁定**：`corpus-guard` 没有第一个 commit 时（`code/` 断链即是信号），唯一允许的学习动作是 L0。禁止修改 `current/`、禁止新增元文档、禁止进入 L0.5 及以后。见 `current/01-guardrails.md` 第 5.1 节。
- 阀门停滞触发期间同样禁止新增「关于怎么学习」的元文档。判断句：文档在描述「我要怎么学」，还是「我从数据里发现了什么」？
